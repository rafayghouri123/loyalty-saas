...
1. **Real Supabase integration:** PostgreSQL 17.6 migrations, runtime-role/pooler grants, actual PostgREST readiness and anonymous denial now pass. Google OAuth is enabled and its authorization endpoint redirects to Google. Completed provider sign-in/callback, JWT/session behavior and the authenticated RPC/outbox slice remain pending.
