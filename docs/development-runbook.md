...
Supabase's private CA may not be in Node's default trust store. Download it from Database Settings → SSL configuration, as described in [Supabase SSL enforcement](https://supabase.com/docs/guides/platform/ssl-enforcement). The verified initial download used the public URL in [official Studio custom content](https://github.com/supabase/supabase/blob/master/apps/studio/hooks/custom-content/custom-content.json). DATABASE_CA_CERT_PATH is read by both runtime pools with rejectUnauthorized=true. Provision that certificate file separately on each host and set its host-local path; the workstation's absolute path is not portable. Do not solve a certificate error by disabling verification.
