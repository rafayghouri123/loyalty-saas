...
-- A shared weekday only conflicts when it occurs inside the overlapping date range.
...
begin
 ...
 if p_enable then
  ...
  where op.business_id=p_business and op.id<>p_promotion and op.status='enabled'
   and other.starts_on<=v.ends_on and v.starts_on<=other.ends_on
   and exists(select from generate_series(greatest(other.starts_on,v.starts_on)::timestamp,least(other.ends_on,v.ends_on)::timestamp,interval '1 day') as overlap_day(day)
    where extract(isodow from overlap_day.day)::integer=any(other.weekdays)
    and extract(isodow from overlap_day.day)::integer=any(v.weekdays))
