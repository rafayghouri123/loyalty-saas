...
export async function POST(request: Request) {
  ...
  let input;
  try { input=z.strictObject({intent:z.enum(['customer','business'])}).parse(await readSmallJson(request)); }
  catch { return failure('invalid_input','Choose a valid sign-in destination.',correlationId); }
  const client=await createUserClient();
  const {data,error}=await client!.auth.signInWithOAuth({provider:'google',options:{redirectTo:`${new URL(config.appUrl).origin}/auth/callback?next=${input.intent==='business'?'/workspace':'/app'}`,skipBrowserRedirect:true}});
