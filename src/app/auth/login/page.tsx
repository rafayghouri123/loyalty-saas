...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ intent?: string; error?: string; next?: string }> }) {
  ...
  return <main id="main" className="auth-wrap"><Link className="wordmark" href="/">{getIdentity().name}</Link><section className="auth-card"><span className="badge">{params.intent==='business'?'For your business':'For your next visit'}</span><h1 style={{marginTop:24}}>Welcome back.</h1><p className="muted">{params.intent==='business'?'Sign in to open your business workspace.':'Sign in to keep your loyalty cards together.'}</p><LoginForm emailConfigured={emailLoginConfigured()} configured={Boolean(getPublicConfig())} business={params.intent==='business'} callbackError={Boolean(params.error)} next={params.next}/></section></main>;
}
